...
is assumed to be UTF-8. This is likely not the case in some
situations, for example with pages encoded in koi-8. There is
no real workaround to that. WWW::Mechanize needs to talk
to Firefox to get its guess about the encoding and then
needs to decode the content according to what Firefox
tells it.
